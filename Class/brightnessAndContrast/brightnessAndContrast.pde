...
void draw() {
  ...
  float contrast = (int)random(1, 5f) * ( mouseX / (float)width); //value should go from 0 to 5
  println("contrast: " + contrast);
  
  //brightness - this is additive and we want to make it both brigher and darker so -128 to +128
  float bright = (int)random(1, 255) * ( mouseY / (float)height  - 0.5); //value should go from -128 to +128
  ...
  //display the image
  image(img, 0, 0);
  image(imgEnhanced, 0, img.height); 
} //draw
